...
Builds a `vendor/extras/` overlay that uses the Pixel product partition's Google Sans Flex as GrapheneOS's default `sans-serif`, and patches the device tree to wire it in.

Default target device: **tokay** (Pixel 9).

## What it does

1. Maps the framework's clock, body, light, regular, and headline families to Google Sans Flex.
2. Maps the system `sans-serif` and Material variable typography families to `GoogleSansFlex-Regular.ttf`.
3. Packages the hand-tuned XML templates in `frozen_fonts/` as `vendor_extras_google_sans_flex.tar.gz`.
4. Extracts that into the AOSP tree and patches the device `.mk` + auto-generated RRO module name.

The font file is already supplied by the Pixel product partition; the overlay does not bundle a duplicate copy.
...
- Run `build_google_sans_flex_vendor.py` to package the overlay.
...
./build_google_sans_flex_vendor.py --check  # validate templates, no build
```
...
build_google_sans_flex_vendor.py # package the Google Sans Flex overlay
...
- The overlay expects `GoogleSansFlex-Regular.ttf` in the Pixel product fonts supplied by the unpacked vendor files.
